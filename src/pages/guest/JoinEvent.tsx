...
// Simple hash generator for session tokens
async function generateSessionHash() {
  const array = new Uint8Array(32)
  crypto.getRandomValues(array)
  const token = Array.from(array, byte => byte.toString(16).padStart(2, '0')).join('')
  // In a real app we might hash this, but for this MVP, the token itself acts as the unguessable identifier
  return token
}
...
export default function JoinEvent() {
  ...
  useEffect(() => {
    async function loadEvent() {
      ...
      try {
        ...
        if (MOCK_MODE) {
          data = mockEvents.find(e => e.public_slug === slug) || mockEvents[0]
        } else {
          const { data: rpcData, error } = await supabase.rpc('get_public_event_info', { p_slug: slug })
    ...
    loadEvent()
  }, [slug, getEventSession, navigate, setSession])
  ...
  const handleJoin = async (isAnonymous: boolean) => {
    ...
    try {
      ...
      const deviceId = localStorage.getItem('ekthau_device_id') || crypto.randomUUID()
    ...
    return (
      <div className="flex h-screen items-center justify-center p-4 text-center">
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground">{error || 'Event not found or has expired.'}</p>
        </div>
      </div>
    )
  }
        ...
        <div className="text-center space-y-3">
          {eventInfo.cover_image_path ? (
            <div className="w-full h-48 rounded-2xl bg-gray-200 overflow-hidden mb-6">
              <img src={eventInfo.cover_image_path} alt="Cover" className="w-full h-full object-cover" />
            </div>
          ) : (
            <div className="w-20 h-20 bg-primary/10 rounded-full mx-auto flex items-center justify-center mb-6">
              <span className="text-3xl font-bold text-primary">{eventInfo.name.charAt(0)}</span>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-sm border space-y-6">
          <div className="space-y-4">
            ...
            <Button 
              ...
              disabled={joining || (!displayName.trim() && !eventInfo.allow_anonymous)}
            >
